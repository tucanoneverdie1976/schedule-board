...
const DOG_IMAGE_REFRESH_MS = 60000;
const DOG_IMAGE_ERROR_RETRY_LIMIT = 5;
const SCHEDULE_SCROLL_PX_PER_SECOND = 24;

let dogImageErrorRetryCount = 0;
let dogImageRequestId = 0;
...
async function loadDogImage() {
  ...
  const requestId = dogImageRequestId + 1;
  dogImageRequestId = requestId;
  els.artImage.classList.add("is-loading");
  try {
    const payload = await request(`/api/dog-image?t=${Date.now()}`);
    const item = payload.item || {};
    if (requestId !== dogImageRequestId || !item.url) {
      throw new Error(payload.error || "Dog image unavailable");
    }
    if (els.artTitle) {
      els.artTitle.textContent = item.title || "Random Dog";
    }
    if (els.artArtist) {
      els.artArtist.textContent = item.source || "Dog CEO Dog API";
    }
    els.artImage.src = item.url;
  } catch (error) {
    els.artImage.classList.remove("is-loading");
    if (els.artTitle) {
      els.artTitle.textContent = "Random Dog";
    }
    if (els.artArtist) {
      els.artArtist.textContent = "이미지를 불러오지 못했습니다";
    }
  }
}
...
if (els.artImage) {
  ...
  els.artImage.addEventListener("error", () => {
    ...
    els.artImage.classList.remove("is-loading");
    if (dogImageErrorRetryCount < DOG_IMAGE_ERROR_RETRY_LIMIT) {
      dogImageErrorRetryCount += 1;
      window.setTimeout(loadDogImage, 1200);
    }
  });
}
...
setInterval(loadDogImage, DOG_IMAGE_REFRESH_MS);
